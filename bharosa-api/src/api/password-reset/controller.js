...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Bharosa Api account.<br>
        ...
        &mdash; Bharosa Api Team
      `
      return sendMail({ toEmail: email, subject: 'Bharosa Api - Password Reset', content })
    })
